    ...
    if(mysqli_num_rows($query_run) > 0)
    {
        foreach($query_run as $row)
        {
            ...
            <tr>
                ...
                if ($row['name'] === 'Admin') {
                    ?>
                    <td><a href="admin_addbook.php?book_id=<?= $row['book_id']; ?>" class="btn btn-success update-btn">Update</a></td>
                    <?php
                } else {
                    ?>
                    <td></td>
       ...
       // Show table with animation when the page loads
        ...
        document.querySelectorAll('.update-btn').forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                var bookId = row.cells[0].innerText;
                // Get other values if needed
                // Perform update operation or redirect to the update page
                window.location.href = "user_addbook.php?book_id=" + bookId;
            });
        });
    </script>
</body>
