...
if ($result_books->num_rows > 0) {
    ...
    $book_details .= "<h3>{$row['book_name']}</h3>";
    $book_details .= "<p>Author: {$row['author']}</p>";
    $book_details .= "<p>Condition: {$row['book_condition']}</p>";
    $book_details .= "<p>Seller Name: {$row['name']}</p>";
    $book_details .= "<p>Seller Email: {$row['email']}</p>";
    $book_details .= "</div>";
    $book_details .= "</div>";
} else {
...
<script>
    ...
    // Close the modal box when clicking on the close button
    span.onclick = function() {
        modal.style.display = "none";
    };

    // Close the modal box when clicking outside the modal
