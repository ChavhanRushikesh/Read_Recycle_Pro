...
<head>
    ...
    <title>graphic-novels-comic</title>
    ...
    border-color: green;
       ...
       <script>
        function addToWishlist(bookId) {
            ...
            form.method = 'post';
            form.action = 'graphic-novels-comic.php'; // Assuming the same file name

            // Create an input element for bookId
        ...
        <div class="col-md-12 card-container">
            ...
            <div class="row">
                ...
                if (isset($_SESSION['email'])) {
                    ...
                    $sql = "SELECT * FROM add_books WHERE category = 'graphic-novels-comic' AND email != '$loggedInUserEmail'";
                } else {
                    // If the user is not logged in, retrieve all non-fiction books
                    $sql = "SELECT * FROM add_books WHERE category = 'graphic-novels-comic'";
                }
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
                   ...
                   echo '<button type="submit" name="add_to_wishlist" class="card-wishlist ' . ($check_result->num_rows > 0 ? 'active' : '') . '">';
                   echo '<i class="fas fa-heart fa-2x"></i>'; // Increase the size of the heart icon
                   echo '</button>';
                   echo '<input type="hidden" name="book_id" value="' . $row['book_id'] . '">';
                   echo '</form>';
               
                   echo '<a href="book_details.php?id=' . $row['book_id'] . '">';
